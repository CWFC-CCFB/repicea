...
class WBirchProdVolTreeImpl implements WBirchProdVolTree {
	...
	WBirchProdVolTreeImpl(int treeID, 
		...
		this.stand = stand;
		this.h20Obs = h20Obs;
		Version version = WBirchProdVolPredictor.getVersion(this, h20Obs);
		
		if (version == Version.Full) {
			this.predRef = new Matrix(6,1);
		...
		} else if (version == Version.FullTruncated) {
			this.predRef = new Matrix(4,1);
			predRef.m_afData[0][0] = h20Pred;
			predRef.m_afData[1][0] = merVolPred;
			predRef.m_afData[2][0] = pulpVolPred;
			predRef.m_afData[3][0] = sawlogVolPred;
		} else if (version == Version.DClass) {
			this.predRef = new Matrix(4,1);
			predRef.m_afData[0][0] = h20Pred;
			predRef.m_afData[1][0] = merVolPred;
			predRef.m_afData[2][0] = pulpVolPred;
			...
			predRef.m_afData[1][0] = merVolPred;
			predRef.m_afData[2][0] = pulpVolPred;
			predRef.m_afData[3][0] = lowGradeSawlogVolPred;
		}
