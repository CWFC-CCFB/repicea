...
public class DBFHeader extends FormatHeader<DBFField> {
	...
	byte month;                  /* 2 */
	byte day;                    /* 3 */
//	int numberOfRecords;         /* 4-7 */			/* now contained in the super class */
	...
	short reserv4;               /* 30-31 */
//	private Vector<DBFField> fieldArray;       /* each 32 bytes */		/* now contained in the super class */
...
//		fieldArray = new Vector<DBFField>();
		
		DBFField field = DBFField.createField(dataInput); /* 32 each */
		while (field != null) {
//			getFieldList().addElement(field);
			addField(field);
...
//		dataOutput.writeInt( numberOfRecords); /* 4-7 */
		setNumberOfRecords(Utils.littleEndian(getNumberOfRecords()));
		dataOutput.writeInt(getNumberOfRecords()); /* 4-7 */
...
//		}
		...
		for (int i = 0; i < getNumberOfFields(); i++) {
			getField(i).write(dataOutput);
//							//System.out.println( "Length: " + fieldArray[i].getFieldLength());
//			((DBFField) field).write( dataOutput);
		}
...
//		(32*fieldArray.length)+
	...
	private short findRecordLength() {
		int recordLength = 0;
		for (int i = 0; i < getNumberOfFields(); i++) {
			recordLength += getField(i).getFieldLength();
		}
	...
	@Override
	protected void setFieldList(List<DBFField> fieldArray) {super.setFieldList(fieldArray);}
