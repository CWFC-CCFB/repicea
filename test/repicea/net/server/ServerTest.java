...
public class ServerTest {
	...
	public void testDistantTCPServerWithSingleRequest() throws Exception {
		...
		int outerport = s.getLocalPort();
		ServerSocket s2 = new ServerSocket(0);
		int internalport = s2.getLocalPort();
		s.close();
		s2.close();
		
		ServerConfiguration configuration = new ServerConfiguration(1, 0, outerport, internalport);
		System.out.println("Configuration instantiated");
		FakeDistantServer server = new FakeDistantServer(configuration);		
		System.out.println("Server instantiated");
		server.startApplication();
		
		InetSocketAddress socketAddress = new InetSocketAddress(InetAddress.getLoopbackAddress(), outerport);
...
//		client.close();
	...
	// TODO move this test in the j4r_server library MF20220317
//	@Ignore
